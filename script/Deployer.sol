...
abstract contract Deployer is Script {
    ...
    modifier broadcastUsingPrivateKey(uint256 privateKey) {
        vm.startBroadcast(privateKey);
        _;
        vm.stopBroadcast();
    }

    modifier broadcastUsingIndex(uint32 index) {
        string memory mnemonic = vm.envString("MNEMONIC");
        (address deployer, uint256 privateKey) = deriveRememberKey(mnemonic, index);
        console2.log("===== broadcasting with private key: ", privateKey);
        vm.startBroadcast(privateKey);
        _;
        vm.stopBroadcast();
    }
    ...
    function setUp() public virtual {
        ...
        console2.log("Connected to network with chainid %s", chainId);
        ...
        console2.log("Storing temp deployment data in %s", tempDeploymentsPath);
    }
    ...
    function sync() public {
        ...
        console2.log("Syncing %s deployments", deployments.length);
        console2.log("Using deployment artifact %s", deployPath);
        ...
        for (uint256 i; i < deployments.length; i++) {
            ...
            console2.log("Syncing deployment %s: contract %s", deploymentName, contractName);

            string[] memory args = getDeployTransactionConstructorArguments(deployTx);
            // issue: https://github.com/foundry-rs/foundry/issues/4049
        ...
        console2.log("Synced temp deploy files, deleting %s", tempDeploymentsPath);
